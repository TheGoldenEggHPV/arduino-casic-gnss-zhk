...
{
  if (!cfgSetMsgRate(io, msg_cls, msg_id, rate))
    return false;
...
{
  if (!beginCfgTxn(0x06, 0x00))
    return false;
  uint8_t payload[1]; // NOTE: Something fishy here as payload lengths should be a multiple of 4 and the spec says a query should have length 0.
  // Notes here: https://github.com/jclark/casictool/blob/main/spec/notes.md#cfg-prt-query
