...
{
  Serial.print(F("speed3D="));
  Serial.print(pv.speed3D_mps, 2);
  Serial.print(F(" m/s  altMSL="));
  Serial.print(pv.altitude_msl_m(), 2);
  Serial.print(F(" m  hAcc1s="));
  Serial.print(pv.hAcc_1sigma_m(), 2);
  Serial.print(F(" m  sAcc1s="));
  Serial.print(pv.speedAcc_1sigma_mps(), 2);
  Serial.print(F(" m/s  SV="));
  Serial.println(pv.numSV);
}

static void printSatSummary(const CasicSatInfo &si)
{
  Serial.print(F("Sat "));
  Serial.print(si.systemType);
...
static void cfgPrint(const __FlashStringHelper *label, CasicCfgResult r)
{
  ...
  switch (r)
  {
  case CasicCfgResult::Ack:
    Serial.println(F("ACK"));
    break;
  case CasicCfgResult::Nak:
    Serial.println(F("NAK"));
    break;
  case CasicCfgResult::Timeout:
...
void setup()
{
  ...
  gps.cfgSetMsgRateAndWait(Serial1, 0xF0, 0x00, 0, 300, &r);
  cfgPrint(F("Disable NMEA GGA"), r);
  gps.cfgSetMsgRateAndWait(Serial1, 0xF0, 0x01, 0, 300, &r);
  cfgPrint(F("Disable NMEA GLL"), r);
  gps.cfgSetMsgRateAndWait(Serial1, 0xF0, 0x02, 0, 300, &r);
  cfgPrint(F("Disable NMEA GSA"), r);
  gps.cfgSetMsgRateAndWait(Serial1, 0xF0, 0x03, 0, 300, &r);
  cfgPrint(F("Disable NMEA GSV"), r);
  gps.cfgSetMsgRateAndWait(Serial1, 0xF0, 0x04, 0, 300, &r);
  cfgPrint(F("Disable NMEA RMC"), r);
  gps.cfgSetMsgRateAndWait(Serial1, 0xF0, 0x05, 0, 300, &r);
  cfgPrint(F("Disable NMEA VTG"), r);

  // Enable CASIC binary messages you care about
  gps.cfgSetMsgRateAndWait(Serial1, 0x01, 0x03, 1, 400, &r);
  cfgPrint(F("Enable NAV-PV"), r);
  gps.cfgSetMsgRateAndWait(Serial1, 0x01, 0x20, 1, 400, &r);
  cfgPrint(F("Enable NAV-GPSINFO"), r);

  // Optional: enable others if your receiver supports them
  gps.cfgSetMsgRateAndWait(Serial1, 0x01, 0x21, 1, 400, &r);
  cfgPrint(F("Enable NAV-BDSINFO"), r);
  gps.cfgSetMsgRateAndWait(Serial1, 0x01, 0x22, 1, 400, &r);
  cfgPrint(F("Enable NAV-GLNINFO"), r);
